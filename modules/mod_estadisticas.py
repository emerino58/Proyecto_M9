...
layout = html.Div([
    ...
    # 📌 Tabla de distribución de jugadores por puesto con tamaño normal
    ...
    dcc.Graph(id="radial-bar-chart")  # 📌 Gráfico Polar de Barras Restaurado
])
...
def update_player_list(selected_team):
    ...
    return player_options, players[0]  

# 🔹 Callback para generar el Gráfico Polar de Barras con líneas más suaves
@callback(
...
def update_radial_chart(selected_team, selected_player):
    ...
    fig = px.bar_polar(
        r=values, theta=categories, title=f"Rendimiento de {selected_player}", color=categories
    )

    fig.update_layout(
        polar=dict(
            bgcolor="white",
            angularaxis=dict(gridcolor="gray", gridwidth=1, linecolor="gray", linewidth=1.5),
            radialaxis=dict(gridcolor="gray", gridwidth=1, linecolor="gray", linewidth=1.5)
        ),
        plot_bgcolor="white",
        paper_bgcolor="white"
    )
