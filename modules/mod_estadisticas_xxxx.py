...
layout = html.Div([
    ...
    # 📌 Tabla de distribución de jugadores por puesto con ancho reducido
    ...
    html.Div([
        dash_table.DataTable(
            ...
            style_table={'overflowX': 'auto', 'width': '60%', 'margin': 'auto', 'border': '1px solid #ccc'},  # 🔹 Ancho reducido y centrado
            style_cell={'textAlign': 'center', 'white-space': 'normal'},  # 🔹 Asegurar que todo el texto se muestre
...
# 🔹 Callback para generar el Gráfico Radial de Barras con líneas más suaves
@callback(
...
def update_radial_chart(selected_team, selected_player):
    ...
    fig = px.bar_polar(r=values, theta=categories, title=f"Rendimiento de {selected_player}", color=categories)
    
    fig.update_layout(polar=dict(gridcolor="gray", gridwidth=1, linecolor="gray", linewidth=1.5))
